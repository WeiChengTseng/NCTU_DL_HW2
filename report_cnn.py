...
f = csv.DictReader(
    open(
        './result/csv/run-CNN_exp_kernel3_stride4_dilation1_test-tag-accuracy.csv',
        'r'))
step, value = zip(*[(float(r['Wall time']), 1 - float(r['Value'])) for r in f])
step = np.array(step) - step[0]
step, value = step[:EPOCH], value[:EPOCH]
...
f = csv.DictReader(
    open(
        './result/csv/run-CNN3_exp_kernel3_stride1_dilation1_test-tag-accuracy.csv',
        'r'))
step, value = zip(*[(float(r['Wall time']), 1 - float(r['Value'])) for r in f])
step = np.array(step) - step[0]
step, value = step[:EPOCH], value[:EPOCH]
...
f = csv.DictReader(
    open('./result/csv/run-SmallCNN5_crop8_wd0.0001_dropout_test-tag-loss.csv',
         'r'))
